...
PASTEBOARD_TRANSIENT_TYPE = "org.nspasteboard.TransientType"
PASTEBOARD_AUTOGENERATED_TYPE = "org.nspasteboard.AutoGeneratedType"
...
def _clipboard_change_count() -> int:
    ...
    try:
        ...
        log(f"Clipboard change count error: {e}")
        return 0
...
def _get_clipboard_text() -> str | None:
    pasteboard = _general_pasteboard()
    if pasteboard is None:
        log("Clipboard error: AppKit NSPasteboard unavailable")
        return None
    try:
...
def _set_clipboard(text: str, transient: bool = False) -> bool:
    """Set clipboard text through NSPasteboard."""
    pasteboard = _general_pasteboard()
    if pasteboard is None:
        log("Clipboard error: AppKit NSPasteboard unavailable")
        return False
    try:
        ...
        if not pasteboard.setString_forType_(text, NSPasteboardTypeString):
            log("Clipboard error: setString failed")
            return False
        if transient:
            pasteboard.setString_forType_("", PASTEBOARD_TRANSIENT_TYPE)
            pasteboard.setString_forType_("", PASTEBOARD_AUTOGENERATED_TYPE)
        return True
    except Exception as e:
...
def _paste_text_preserving_clipboard(text: str, paste_func, type_func, sleep_func=time.sleep) -> bool:
    ...
    if not _set_clipboard(text, transient=True):
        log("Clipboard failed - falling back to typing")
        type_func(text)
        return False

    temporary_change_count = _clipboard_change_count()
    sleep_func(0.05)
    paste_func()
    log("Pasted via clipboard")
    sleep_func(0.15)

    current_change_count = _clipboard_change_count()
    if temporary_change_count and current_change_count and current_change_count != temporary_change_count:
